...
class Node:
    def __init__(self, name, dodocoin, connected_node=[]):
        self.node_name = name
    ...
    def connect_with_new_node(self, node, sync_chain=False):
        ...
        if node not in (self.connected_nodes):
            #self.connected_nodes.pop(0)#done here 
    ...
    def create_new_block(self):
        ...
        new_block = Block(index=len(self._chain), transactions=self.cryptocurrency.mem_pool,
                          previous_block_hash=self._chain[-1].block_hash, current_version=self.cryptocurrency.current_version,difficulty_level=self.cryptocurrency.difficulty_level,
                          metadata='')
        
        new_block.generate_hash()
        previous_block_hash = new_block.get_previous_block_hash()
        if self.validate_block(new_block,previous_block_hash):
    ...
    def validate_block(self, new_block,previous_block_hash):
        
        hash_block = self._chain[-1].block_hash  # Use self to access the current instance's attribute
        if previous_block_hash == hash_block:  # Correct the variable name
            return True
